...
(function() {
    return {
        ...
        //TODO: this should  be gone when proper servers are implemented
        server:              process.env.STATSD_SERVER,
        address:             process.env.STATSD_ADDRESS || "0.0.0.0",
        address_ipv6:        process.env.STATSD_ADDRESS_IPV6 == "true",
        port:                parseInt(process.env.STATSD_PORT) || 8125,

        debug:               process.env.STATSD_DEBUG == "true",
        mgmt_address:        process.env.STATSD_MGMT_ADDRESS || "0.0.0.0",
        mgmt_port:           process.env.STATSD_MGMT_PORT || "8126",
        title:               process.env.STATSD_TITLE == "false" ? false : process.env.STATSD_TITLE || "statsd",
        healthStatus:        process.env.STATSD_HEALTH_STATUS == "down" ? "down" : "up",
        dumpMessages:        process.env.STATSD_DUMP_MESSAGES == "true",
        flushInterval:       parseInt(process.env.STATSD_FLUSH_INTERVAL),
        // TODO: percentThreshold:
        flush_counts:        process.env.STATSD_FLUSH_COUNTS != "false",

        keyFlush: {
            interval:        parseInt(process.env.STATSD_KEY_FLUSH_INTERVAL) || 0,
            percent:         parseInt(process.env.STATSD_KEY_FLUSH_PERCENT) || 100,
            log:             process.env.STATSD_KEY_FLUSH_LOG || "STDOUT"
        },

        deleteIdleStats:     process.env.STATSD_DELETE_IDLE_STATS == "true",
        deleteGauges:        process.env.STATSD_DELETE_GAUGES == "true",
        deleteTimers:        process.env.STATSD_DELETE_TIMERS == "true",
        deleteSets:          process.env.STATSD_DELETE_SETS == "true",
        deleteCounters:      process.env.STATSD_DELETE_COUNTERS == "true",
        prefixStats:         process.env.STATSD_PREFIX_STATS || "statsd",

        keyNameSanitize:     process.env.STATSD_KEY_NAME_SANITIZE != "false",
        console: {
            prettyprint:     process.env.STATSD_CONSOLE_PRETTYPRINT != "false"
        },
        ...
        graphite: {
            globalPrefix:    process.env.STATSD_GRAPHITE_GLOBAL_PREFIX || "stats",
            legacyNamespace: process.env.STATSD_GRAPHITE_LEGACY_NAMESPACE != "false",
            prefixCounter:   process.env.STATSD_GRAPHITE_PREFIX_COUTNER || "counters",
            prefixTimer:     process.env.STATSD_GRAPHITE_PREFIX_TIMER || "timers",
            prefixGauge:     process.env.STATSD_GRAPHITE_PREFIX_GAUGE || "gauges",
            prefixSet:       process.env.STATSD_GRAPHITE_PREFIX_SET || "sets",
            globalSuffix:    process.env.STATSD_GRAPHITE_PREFIX_TIMER
        },
        ...
        automaticConfigReload: process.env.STATSD_AUTOMATIC_CONFIG_RELOAD != "false"
    };
})()
